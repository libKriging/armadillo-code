...
glue_solve_gen::apply(Mat<eT>& out, const Base<eT,T1>& A_expr, const Base<eT,T2>& B_expr, const uword flags)
  ...
  const bool no_warn      = bool(flags & solve_opts::flag_no_warn     );
  ...
  if(no_warn     )  { arma_extra_debug_print("no_warn");      }
  ...
  const bool allow_warn = (no_warn == false);
  ...
  if(A.n_rows == A.n_cols)
    ...
    if( (status == true) && (rcond > T(0)) && (rcond < auxlib::epsilon_lapack(A)) )
      {
      if(allow_warn)  { arma_debug_warn("solve(): solution computed, but system seems singular to working precision (rcond: ", rcond, ")"); }
      }
    ...
    if( (status == false) && (no_approx == false) )
      ...
      if(rcond > T(0))
        {
        if(allow_warn)  { arma_debug_warn("solve(): system seems singular (rcond: ", rcond, "); attempting approx solution"); }
        }
      else
        {
        if(allow_warn)  { arma_debug_warn("solve(): system seems singular; attempting approx solution"); }
        }
